package com.BaseProject.utils.Aspect;
...
public class AspectClass {
    ...
    @Before("execution(* com.BaseProject.wallet.service.*.*(..))"+
            "|| execution(* com.BaseProject.user.service.*.*(..))"
    )
    public void logBeforeMethod(JoinPoint joinPoint) {
        logger.info("Method called : " + joinPoint.getSignature().toShortString());
    }

    @AfterThrowing("execution(* com.BaseProject.wallet.service.*.*(..))"+
            "|| execution(* com.BaseProject.user.service.*.*(..))"
    )
    ...
    @Around("execution(* com.BaseProject.wallet.service.*.*(..))"+
            "|| execution(* com.BaseProject.user.service.*.*(..))"
    )
